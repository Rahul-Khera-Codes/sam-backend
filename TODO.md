...
**WS8 — compose/send NEW email:** ✅ fixed in code (commit pending push), pending live verify. `send_email_draft` required `email_id` → composing a new email failed Pydantic validation; made it optional + added `draft_email(to,subject,body)` preview tool. Spec `docs/superpowers/specs/2026-06-25-executive-agent-compose-email.md`. **Test:** "draft an email to X saying …" → preview → "yes" → sends.
