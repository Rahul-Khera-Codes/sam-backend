...
Last updated: 2026-07-01 (Session 55 — Avatar toggle + attach-doc bug fix + center buttons + CLAUDE.md dev process rules. Branch `feature/exec-agent-improvements` (both repos). Completed: (1) CLAUDE.md Rule 6 + verify→spec→implement dev process; (2) Start Session button centered in AgentDisplay; (3) Unmute Mic button centered below avatar; (4) attach-doc-to-email BUG FIXED — document library at startup, list_documents tool, MIMEBase PDF in send flow; (5) avatar toggle button in header — avatar_enabled through full stack (backend→dispatch→agent→hook→UI), localStorage persisted, locked during session, drop-safe. Prev session 54: HeyGen LiveAvatar Phase-2 live (marin voice). Branch: `feature/exec-agent-improvements`. PENDING: Docker restart for attach-doc changes; live-verify WS4/10/11/12/13 + avatar toggle; apply migration 20260618000000; merge feature/exec-agent-improvements → feature/google-calendar-timezone → main.)
...
- [x] **Cost estimate for Sam** (owed). ✅ DONE Session 55 — sent via Upwork. HeyGen LITE ≈$0.10/min, OpenAI Realtime ≈$0.20–0.40/min, LiveKit ≈$0.01/min. Avatar adds ~$1/session, ~$30/mo at moderate usage.
