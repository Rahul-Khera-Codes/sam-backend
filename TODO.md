...
**WS3 — rich cards Phase A:** ✅ email_list card LIVE-VERIFIED (renders real emails + Remi summarises briefly); calendar_schedule same mechanism, quick to confirm. `email_list` (list_emails) + `calendar_schedule` (get_schedule) emit via `_send_card` → render as cards above the input (`AgentCardView` registry + text fallback); agent speaks a one-line summary. Envelope `{type:card,card,id,data}` + `card_dismiss`. **Phase A.2:** ✅ DONE in code (pending live verify) — email_draft + calendar_event previews now ride the unified card envelope (`{type:card, card:email_draft|calendar_event_preview, ephemeral, data, actions}`); single `activeCard` slot via `AgentCardView` (old preview panel removed); approval keeps synthetic-text path; `card_action` round-trip deferred to Phase B. Commits backend `8015f11` / frontend `43639c3`. **Test:** "draft a reply / new email" + "block off Friday 10am" → card with Send/Create + Cancel buttons → approve sends/creates. **Phase B:** email_detail, free_slots pick-to-book, appointment_list actions (this is where `card_action` gets built). **Test:** "list my emails" / "what's on my calendar today" → card renders, Remi summarises briefly.
