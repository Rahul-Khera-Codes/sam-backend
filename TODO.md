...
- [x] **Idle-session auto-disconnect.** ✅ DONE 2026-07-02 — listens to LiveKit's built-in `user_state_changed`, hangs up after 3 min of sustained "away" state. Commit `f8972ee`. **Live-tested → found + fixed a real bug (commit `fac4547`):** `ctx.room.disconnect()` only detaches the agent's own participant, not the frontend's separate WebRTC connection — frontend UI never noticed. Fixed by deleting the room server-side via `ctx.api.room.delete_room(...)`, which force-disconnects every participant. **Still pending live-verify:** (1) the room-delete fix itself, (2) whether text-only (mic-off) sessions also trigger `user_state_changed` correctly.
