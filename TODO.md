...
### Billing Add-On Toggle for Executive Agent — IMPLEMENTED, PENDING MIGRATION (2026-07-02)
...
- [ ] **⚠️ Migration not yet applied — backend restart confirmed the error:** `column businesses.stripe_exec_agent_item_id does not exist`. Needs `supabase db push` (or manual run of `20260702000000_businesses_exec_agent_addon.sql`) before this works at all. NOT run automatically — DB is shared between local/server, asked Rahul to confirm before running.
- [ ] **Not yet live-tested** — needs the migration applied first, then a real test: toggle on/off in Billing UI, confirm Stripe subscription item created/removed, confirm `/dashboard/executive` still accessible (enforcement is off by default, so nothing should be blocked yet).
