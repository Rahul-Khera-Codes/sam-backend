...
Last updated: 2026-09-03 (on `feature/hr-agent`, both repos — built the Outlook OAuth connect flow end-to-end (Linear AIE-34), replacing the old UI stub, after the client provided a real Azure AD app registration. Mirrors the existing Gmail integration shape exactly: backend router/service/config, `outlook_tokens` Supabase migration (not yet pushed), frontend API client + IntegrationsTab wiring. Deliberately scoped to connect-flow only — not wired up as an actual email sender yet. 3 manual steps remain before it works end-to-end: fix a redirect-URI typo in the Azure app, paste the real client secret into `backend/.env`, and push the migration. Full detail in `docs/features/outlook-integration.md` and `docs/SESSION_HANDOFF.md`.)

Previously updated: 2026-07-19 (on `feature/hr-agent`, both repos — reviewed the HR Employee scaffolding: 1 backend commit (single hardcoded `GET /hr/mock-workspace` mock endpoint, no DB/real logic yet) + 1 frontend commit (~3000 lines, 5 real pages matching the 9-screen PDF, faithful UI shell with local view-switching only, no create/edit/delete/search wired yet, no Greenhouse calls) — a reasonable Phase-1 milestone, no issues found. Reviewed the Greenhouse Integration Proposal doc + Sam's detailed confirmed reply (full content in `docs/CLIENT_COMMS_LOG.md`). Found + verified against Greenhouse's own docs: Harvest API v1/v2 deprecates 2026-08-31 (~6 weeks out) — must target Harvest v3 (OAuth, per-customer client-credentials) not the older static-key style; Job Board API itself is unaffected, confirmed self-serve. Also surfaced that Sam independently found and applied for Greenhouse Partner status (2026-07-17, before HR build work started) specifically to solve the LinkedIn/Indeed distribution wall found in the Marketing-vs-HR research — Rahul is the listed technical contact and should check for a response, since approval would also resolve Greenhouse's sandbox-access gate (normally a paying-customer-only benefit).)

Previously updated: 2026-07-17 (Session 64 — reviewed Session 63's 7 Cursor-built commits against Sam's original requests: solid overall, 3 minor non-blocking follow-ups logged below (competitor-edit cross-platform URL validation gap, a client-message/UI mismatch on competitor name editing, untracked `supabase/.temp/` noise needing a `.gitignore` rule). Researched Marketing-vs-HR build priority in depth — decomposed both into modules, researched current LinkedIn/Indeed/Meta/TikTok/X third-party API access requirements via parallel research agents. Finding: HR's LinkedIn/Indeed job-board module isn't just slow, it's currently unavailable at Sam's scale (LinkedIn Jobs API closed to new partners, Indeed requires ≥10 paying clients to apply, LinkedIn candidate-sourcing has no self-serve path and the scrape-workaround carries real legal risk — LinkedIn sued and shut down Proxycurl for this in 2025); HR's other 3 modules have zero external-platform dependency and are buildable today; Marketing's integrations are all genuinely self-serve-with-review but the product needs all of them before it does anything real. Recommendation (`docs/next-employee-build-recommendation.md`) sent to Sam via Google Doc, not yet reviewed by him. Shipped 2 more Branding fixes from Sam's latest requests: Communication Strategy is now a text field (dropped unused `use_emojis`), Market Insights section removed (dropped unused `emerging_trends`, kept + relocated `target_niche` into Core Brand since it feeds Market Agent's industry-relevance fix — deleting it would have silently regressed that). Verified live via DOM checks, a direct DB row query, and a direct call to `_build_industry_context()` confirming the fix still works post-migration.)

Previously updated: 2026-07-16 (Session 63 — implemented two Market Agent UX features on `feature/sales-lead-researcher` in both repos. Added a `View Report` action on every Market Agent card, backed by a new report dialog showing the full insight, prompt used, generated timestamp, and all stored source links. Added custom-report editing end-to-end: backend `PATCH /sales/market-agent/custom-analysts/{id}`, frontend reuse of the custom-analyst dialog in edit mode, and a stable `custom_analyst_id` + `prompt_used` data path on card records so the edit action can reliably open from the card/report view. Added migration `20260716000000_market_agent_card_prompt_linkage.sql` to persist that linkage and backfill uniquely matchable historical custom cards. Verified with `npx tsc --noEmit`, targeted Python AST parsing of the changed backend files, and clean IDE lints on all touched files.)

Previously updated: 2026-07-14 (Session 61 — branch `fix/sonner-toast-notifications` both repos, just created, no work started yet. Ran a full real-user re-test of all 4 Sales Employee modules + Business Branding: 26 tests across 5 parallel Canary sessions, all passing. Headline result: TC-MA-INTEGRATION-001 confirmed with quoted evidence that Market Agent's reports now genuinely reflect Branding's target_niche — real named competitors, trade-press citations, zero generic content, and backend logs proving the fetch-order wiring. One regression found and fixed same session: Report Scheduler's send-test failed due to an expired Gmail OAuth token for the test business (not a code bug) — reconnected and re-verified sent:true. Confirmed the Competitor Agent OpenAI-key issue isn't present locally (though not directly re-tested against production). Merged `feature/business-branding` → main in both repos (clean fast-forward, backend health + frontend build both verified). Created a fresh branch for the one remaining bug — the app-wide Sonner toast issue — kept deliberately separate per the user's request. PENDING: fix the toast bug; real production deploy; Sam's lawyer sign-off; live-test the Executive Agent billing add-on toggle now that a base plan exists.)

Previously updated: 2026-07-13 (Session 60 — branch `feature/business-branding` both repos, pushed, not yet merged. Merged everything to `main` (both repos now fully up to date). Ran a full live E2E pass on the DEPLOYED site covering Billing/Sales Employee/Remi/Report Scheduler — all confirmed working — and found + fixed 2 more real bugs same-day: Remi's calendar mislabeled multi-day ranges as "today" and showed events without dates (recurring meetings looked triple-booked); Report Scheduler's live preview didn't update on unsaved toggles. Investigated 3 issues Sam reported: Lead Researcher "changing window stops analysis" (root cause: local state destroyed on tab switch, backend job unaffected — fixed by making the History tab self-poll while anything is running instead); Competitor Agent "Discovery Failed" (root-caused to an invalid OpenAI key on the deployed server, since fixed by Rahul, not yet re-verified); Market Agent generic reports (root-caused to `businesses.type` defaulting to vague values like "other"). Built the Business Branding feature end-to-end per Sam's request (spec → new `business_branding` table, applied live → backend CRUD → Market Agent now uses `target_niche` for industry context, with safe fallback → full `BrandingTab.tsx` UI) — verified live including a page-reload persistence check. Found a real, pre-existing, APP-WIDE bug while verifying: toast confirmations (`sonner`) silently never appear anywhere in the app — confirmed via an old, untouched Report Scheduler save, so not caused by today's work; root cause not yet found after ruling out several likely causes. PENDING: fix the toast bug (dedicated investigation needed); merge `feature/business-branding` to main once Sam reviews; real production deploy; Sam's lawyer sign-off.)

Previously updated: 2026-07-08 (Session 59 — branch `feature/sales-lead-researcher` both repos. Full end-to-end QA of all 4 Sales Employee modules via real browser (Canary/Playwright) against the live frontend + backend + real external APIs — 25 tests, `docs/sales-employee-qa-test-sheet.md`. 22 passed; found 3 real bugs backend-only testing had missed: no URL format validation anywhere (Lead Researcher — a real paid Apify run started for garbage input), sparse-data indicator regex never matched real backend prose (Competitor Agent), "What's Changing" banner never populated on a normal page load (Market Agent). Fixed and re-verified all 3 live in the same session: matching frontend+backend LinkedIn-URL validators; replaced the fragile regex with an AI-judged `data_availability` field returned directly by the synthesis LLM; `MarketAgent.tsx` now fetches the latest run's stored summary via `run_id` on every page load. All 25 test cases now pass. QA docs (`QA_FINDINGS.md`, `qa_state.md`, test sheet) closed out and committed. PENDING (unchanged from session 58): apply the updated_at-trigger migration; set up a persistent ngrok static domain (waiting on Rahul to reserve one via dashboard.ngrok.com); real production deploy.)

Previously updated: 2026-07-07 (Session 58 — branch `feature/sales-lead-researcher` both repos. Built and live-verified all 4 Sales Employee backend modules end-to-end: Lead Researcher, Competitor Agent, Market Agent, Report Scheduler — every one surfaced and fixed at least one real bug during live testing (footer-truncation, legacy YouTube URLs, location-scoped Gmail tokens, XSS-in-email, IDOR risk, secrets-in-URLs). Sam confirmed Exa.ai for Market Agent (resolved Yuvraj's 4 open scoping questions). API contract docs written for all 4 modules, handed to Yuvraj. Real bug report from Yuvraj's own testing investigated and root-caused (Apify webhook delivery failing due to local ngrok tunnel being down — not a code bug); 2 stuck lookups manually recovered from Apify's already-succeeded dataset; found+fixed a separate real bug (updated_at never auto-updates on any of the 8 new tables — fixed via DB trigger, migration written, NOT YET APPLIED). Sam added Rahul+Yuvraj to his real Apify org (personal Apify token now swapped, verified working) and his Exa.ai org. PENDING: apply the updated_at-trigger migration; set up a persistent ngrok static domain (task in progress, waiting on Rahul to reserve one via dashboard.ngrok.com); real production deploy (still ngrok+personal-testing-account patterns in places).)

Previously updated: 2026-07-06 (Session 57 — Sales Employee requirements doc converted to docx and sent to the group (Yuvraj/Sam/Charles); billing add-on live-test on hold, Rahul will provide test details later.)

Previously updated: 2026-07-02 (Session 56 — huge session, branch `feature/exec-agent-improvements` both repos. Shipped: greeting fix, editable email draft (WYSIWYG-locked send, same bug class WS0 fixed), transcript edit/copy buttons, avatar default-off, idle-session auto-disconnect (+ found/fixed a real bug — room.disconnect() only left the agent's own participant, not the frontend's), prompt-cache audit (~70% hit rate measured live → decided to stay on OpenAI Realtime, not build a separate STT/LLM/TTS pipeline). Full billing add-on for Executive Agent built end-to-end per new ADR 0001 (Stripe subscription line item, two-layer gating, enforcement OFF by default) — migration applied, then found+fixed a real bug: the toggle was invisible for every business since none has a real Stripe subscription. Found + logged (not fixed, deliberately deferred) a bigger whole-product billing-enforcement gap. Sam confirmed: start Sales Employee next (Yuvraj on UI) — wrote + re-verified full requirements doc against the PDF mockups (found 2 real gaps, fixed), added platform-integration-requirements section (LinkedIn/Facebook/Instagram/YouTube via Apify, news aggregation as a separate cost). Wrote voice-agents cost breakdown doc + per-provider cost appendix. Drafted (not sent) ChatGPT-scope questions for Sam. Logged Sam's calendar OAuth-mismatch bug report (Rahul handling himself) + billing UI question. PENDING: live-test billing add-on with a real Stripe subscription; reconcile `fix/avatar-aec`; apply timezone migration; merge to main; Sales Employee Lead Researcher build (needs Apify account).)
...
### Session 64 Follow-ups (2026-07-17, from code review of Session 63's Cursor commits)
- [ ] **Competitor Agent edit-links validation gap** — neither `UpdateCompetitorRequest` (backend) nor the edit dialog (frontend) checks that a URL actually belongs to the platform's field (e.g. a YouTube link pasted into the LinkedIn field is silently accepted both sides). Reuse the existing platform-specific regex pattern already used for `LeadLookupRequest.linkedin_url`.
- [ ] **Competitor edit — name field missing from UI** — Sam was told (Jul 16 client update) that he can edit a competitor's "name and social profile links." The backend supports editing the name via the same `PATCH` endpoint, but the shipped dialog (`MonitoredCompetitorsList.tsx`) only has the 4 social-link inputs, no name field. Either add the field or correct what Sam was told.
- [ ] **`ai-employees-app/supabase/.temp/` tracked in git with no `.gitignore` rule** — contains CLI version files + `linked-project.json` (Supabase project ref, org id, project owner's email — not a secret key, but shouldn't be tracked). Recurs on every commit that touches Supabase locally. One-time fix: add `supabase/.temp/` to `.gitignore`, untrack the directory.
- [ ] **Marketing vs HR build-order decision** — `docs/next-employee-build-recommendation.md` sent to Sam via Google Doc (2026-07-15), not yet reviewed by him as of 2026-07-17. See `docs/SESSION_HANDOFF.md` Session 64 for the full reasoning (HR's 3/4 modules unblocked, LinkedIn/Indeed job-board module currently unavailable at Sam's scale; Marketing's integrations all self-serve but the product needs all of them). Open question from Rahul about recommending Marketing over HR instead — no technical justification found yet, would need a stated business reason.
...
- [x] **`POST /phone-numbers/sync-dispatch`** — verified session 62 (2026-07-14) via direct LiveKit API call (`list_sip_dispatch_rule`): every dispatch rule belonging to a currently-existing business already carries `location_id` in its attributes. The one rule without it (`SDR_VURGm7seCXam`) belongs to a business no longer present in the `businesses` table — an orphaned/deleted-business artifact, not a live gap. Nothing left to sync; endpoint doesn't need to be run.
...
- [x] **Migrations + edge functions** — verified session 62 (2026-07-14): `supabase migration list` confirms `20260428000001/2/3` all applied on remote; `supabase functions list` confirms `invite-location-admin` (v17) + `accept-invitation` (v11) both `ACTIVE`, last deployed 2026-04-29. Stale checkbox — this was done in an earlier session without being marked.
...
- [x] **#2 Greeting without company name.** ✅ DONE 2026-07-02 — `executive_agent.py:1228` greeting instruction now says `"Hi, I'm Remi — how can I help you today?"`, no business name. Identity-question answer ("I'm Remi, your assistant for {business_name}") left unchanged — separate, correct behavior. Commit `a8c7e72`.
- [x] **#3/#4/#5 Editable email draft card (body / recipient / subject).** ✅ DONE 2026-07-02. Fixed the same class of bug WS0 fixed for calendar events: `send_email_draft` now prefers `self._pending_draft` over LLM-retyped tool-call args (commit `bbe0527`); new `send_email_draft` card_action writes the edited to/subject/body into the pending preview before the model acts, so the send is WYSIWYG-locked to the card. Frontend: `email_draft` card is now an editable form with Send/Cancel (commit `351c4d4`). Needs a live test with an actual typo fix before considering fully verified.
...
- [x] **#7 Edit + copy buttons on transcript bubbles.** ✅ DONE + LIVE-VERIFIED 2026-07-02 (frontend only, `ai-employees-app`). Copy button on every bubble, user + agent (commit `1321b0f`). Edit button on user bubbles only — resends the corrected text as a new message via `sendMessage` rather than rewriting history (commit `ccdf233`). **Rahul confirmed working** — original bubble stays in history, correction sends as a new bubble, Remi responds naturally to it, copy icon appears on hover on both user and agent bubbles.
...
- [x] **Avatar default-off.** ✅ DONE 2026-07-02 — `backend/app/routers/executive.py:29` + `useExecutiveSession.ts:114-121` both flipped to `False`. Existing users' localStorage preference untouched. Commits `e55bae8` (backend) / `1272c0b` (frontend).
- [x] **Idle-session auto-disconnect.** ✅ DONE + LIVE-VERIFIED 2026-07-02 — listens to LiveKit's built-in `user_state_changed`, hangs up after 3 min of sustained "away" state. Commit `f8972ee`. First attempt used `ctx.room.disconnect()` which only detached the agent's own participant, not the frontend's separate WebRTC connection — frontend UI never noticed. Fixed by deleting the room server-side via `ctx.api.room.delete_room(...)` (commit `fac4547`), which force-disconnects every participant. **Rahul confirmed working** — frontend now correctly drops to disconnected state. **Still open:** whether text-only (mic-off) sessions also trigger `user_state_changed` correctly hasn't been separately tested (only voice tested so far).
- [ ] **Remaining:** test `gpt-realtime-mini` → scope-narrowing conversation with Sam (still the biggest lever on cost AND his "why pay vs ChatGPT" complaint).
...
- [x] **Full requirements doc written + re-verified against the PDF** (2026-07-02) — `docs/sales-employee-agenticbi-requirements.md`, simple English, for Yuvraj/Sam/Charles. Covers all 4 screens in detail, the confirmed pipeline, what's NOT included (CRM, cold-calling, legal sign-off), known risks, and **what's actually required to integrate LinkedIn/Facebook/Instagram/YouTube/news sources + real usage-based costs** (commit `1e9cfff`). Re-check against the mockups found 2 gaps, both fixed: Competitor Agent tracks 4 named platforms not generic "social media"; Market Agent has an "Add Custom Report" feature that was missing entirely. Converted to docx and **sent to the group (Yuvraj/Sam/Charles) on 2026-07-06.**
...
- [x] **Apify data source live-verified (2026-07-06)** — tested `data-slayer/linkedin-profile-scraper` on real profiles using Rahul's personal (temporary/testing-only) Apify account. Base scrape: ~$0.015/profile, few seconds, rich accurate data. With "find verified work email" on: ~$0.03/profile but **2+ min latency** and weak `catch_all`-style confidence. Also researched non-Apify alternatives (PDL, Apollo.io, Lusha, RocketReach, Hunter.io) — best alternative found (PDL + Hunter.io, ~$0.30-0.35/lead, synchronous, lower legal risk) **not** adopted since Sam explicitly directed Apify as the data source (2026-06-24); logged as a fallback option only. Full detail: `docs/lead-researcher-data-source-verification.md`.
- [x] **Resolved (2026-07-06/07):** Sam added Rahul + Yuvraj to both his **Apify organization** and his **Exa.ai organization** (confirmed access 2026-07-07 AM). Personal Apify account no longer needed — swap `APIFY_API_TOKEN` in `.env` to a new token generated under the org account.
- [x] **Sam confirmed: Market Agent will use Exa.ai** (2026-07-06, in response to Rahul's direct confirmation question) — not the traditional news-API list (Currents/TheNewsAPI/Mediastack/GNews) originally being evaluated. Sam also uploaded a "setup prompt file" — **need to review its content before speccing Market Agent.**
- [x] **Lead Researcher backend MVP implemented + verified end-to-end (2026-07-06)** — new branch `feature/sales-lead-researcher` (both repos). Async flow: `POST /sales/lead-researcher/lookup` starts an Apify actor run with a webhook, `POST /webhook` (Apify's callback) fetches the dataset + runs a gpt-4o-mini enrichment pass into the card fields, `GET /lookup/{id}`/`GET /history` let the frontend poll. New `lead_lookups` table, migration applied. Live-tested 3x via curl + a real ngrok tunnel — all completed correctly. Security review caught secrets-in-URL issues (webhook secret, Apify token) — fixed, moved to headers. Full detail: `docs/superpowers/specs/2026-07-06-sales-lead-researcher.md`.
- [x] **Follow-up gaps fixed (2026-07-06):** duplicate-request protection (reuses in-flight lookup instead of double-charging Apify) and stuck-job timeout (10 min, marks `failed` instead of polling forever) both implemented + verified against the running container (commit `72c3a0e`). RLS migration (`20260706000001_lead_lookups_rls.sql`) applied to the DB and verified enforcing (anon key sees 0 rows, service role sees the real 3).
- [x] **API contract for frontend written** — `docs/lead-researcher-api-contract.md`. Yuvraj can start the Lead Researcher UI.
- [ ] Production still needs real (non-personal, non-ngrok) Apify + webhook env values before this goes live for real users.
- [x] **Competitor Agent backend implemented + verified end-to-end (2026-07-06)** — same branch. Fan-out design: `POST /competitors` discovers LinkedIn/Facebook/Instagram/YouTube from a website URL (Jina Reader + GPT, SSRF-safe); `POST /competitors/{id}/report` fans out to up to 4 platforms in parallel (3 via Apify + webhooks, YouTube via the official free Data API v3) with a concurrency-safe join step before synthesizing. 3 new tables + RLS from the start this time. Live-tested against a real competitor (hubspot.com) — found + fixed 2 real bugs (a footer-truncation bug that silently dropped every social link, and an unhandled legacy YouTube URL format). API contract written: `docs/competitor-agent-api-contract.md`. Full detail: `docs/superpowers/specs/2026-07-06-sales-competitor-agent.md`.
- [x] **Market Agent backend implemented + verified end-to-end (2026-07-07)** — same branch. Sam confirmed Exa.ai (not the traditional news-API list) after Rahul's direct scope-check; 4 scoping questions from Yuvraj's doc resolved (ownership=Sales Employee confirmed already, purpose=the 7 already-scoped analyst types, trigger=daily scheduled + manual refresh, execution=background job not real-time — no voice/chat in this product). 7 cards per refresh: 6 via Exa's `/search` with `outputSchema`+grounding (live-verified against a real query before building — citations work exactly as documented), 7th "Business Intelligence" card reads this business's own `analytics.py` summary instead of the web (that analyst type is explicitly about internal data per the original spec). All run concurrently via `asyncio.gather` — Exa is synchronous (~40s worst case), no webhook infra needed unlike Apify. New daily APScheduler job added to `scheduler_service.py`. 3 new tables + RLS from the start. Live-tested against a real business — all 7 cards completed cleanly on the first try, no bugs found (unlike the other 2 modules). API contract: `docs/market-agent-api-contract.md`. Full detail: `docs/superpowers/specs/2026-07-07-sales-market-agent.md`.
- [x] **Report Scheduler backend implemented + verified end-to-end (2026-07-07)** — same branch. Last of the 4 Sales Employee modules — **all 4 now shipped.** No external API needed (pure aggregation of the other 3 modules' existing data), sent via the business's existing connected Gmail integration (same `send_email` path as appointment confirmations). CRUD + `/preview` (synchronous, no polling) + `/send-test` (sends to the requester's own email, doesn't affect `last_sent_at`). New hourly sweep job in `scheduler_service.py` (`run_report_scheduler_digests`) checks every active schedule against its daily/weekly/monthly frequency. Live-tested: created a real schedule, previewed it, and confirmed real email delivery via Gmail. Found + fixed 2 real issues during testing: (1) Gmail token lookup was location-scoped and found nothing for a business whose Gmail connection was tied to a specific location, not business-wide — added a location-agnostic lookup; (2) security review caught HTML/XSS injection risk in the digest (content indirectly sourced from scraped competitor posts + Exa search results) — fixed with HTML escaping + recipient email validation. API contract: `docs/report-scheduler-api-contract.md`. Full detail: `docs/superpowers/specs/2026-07-07-sales-report-scheduler.md`.
- [x] **Sales Employee backend build — ALL 4 MODULES COMPLETE (2026-07-07).** Apify org access resolved — Sam added Rahul+Yuvraj to his real Apify org and Exa.ai org; `APIFY_API_TOKEN` swapped to the new token, verified valid/working. Remaining before real users: production deploy (still ngrok for the Apify webhook, not a real domain), Yuvraj's UI build against the 4 API contract docs, Sam's lawyer sign-off on scraped-lead outreach (CASL/ToS), news integration still deferred (Market Agent uses Exa instead, Competitor Agent's "news" section not built).
- [x] **Real bug from Yuvraj's own testing (2026-07-07), root-caused + fixed:** 2 Lead Researcher lookups failed with "Timed out waiting for Apify — the webhook never arrived." Root cause confirmed via Apify's webhook-dispatch API: the actual Apify scrapes **succeeded**, but delivery got HTTP 404 **from ngrok itself** (tunnel not running at that moment) — not a code bug. Both lookups manually recovered from Apify's already-succeeded dataset (no data lost) — `6eb0698e-...` (Arun Kumar) and `efa66db2-...` (Yuvraj Singh) now show `status: completed`. Persistent ngrok static domain — resolved (2026-07-10): Rahul confirmed the existing local ngrok setup already doesn't change on reconnect, no purchase needed.
- [x] **Full end-to-end QA pass + same-session fixes (2026-07-08, Session 59)** — 25 tests via real browser (Canary/Playwright) against all 4 modules, `docs/sales-employee-qa-test-sheet.md`. 22 passed; 3 real bugs found and fixed same session: (1) no URL format validation anywhere — added matching frontend (`NewAnalysisTab.tsx`) + backend (`schemas/sales.py` Pydantic validator) LinkedIn-URL checks; (2) sparse-data regex in `CompetitorReportDialog.tsx` never matched real backend prose — replaced with an AI-judged `data_availability` field returned directly by the synthesis LLM (`competitor_agent.py` + schema); (3) "What's Changing" banner blank on page load — `MarketAgent.tsx` `fetchCards()` now fetches the latest run's summary via `run_id` on load. All 3 re-verified live. All 25 test cases now pass. Full detail + commit hashes: `docs/QA_FINDINGS.md` Resolved Failures.
- [x] **Merged everything to `main` (2026-07-09, Session 60)** — both repos, clean fast-forward + merge, production build verified before pushing. `main` is now fully current.
- [x] **2 more real bugs found + fixed on live production (2026-07-09, Session 60)** — Remi's calendar mislabeled multi-day ranges as "today" and showed events without dates (a recurring meeting looked triple-booked on one day); Report Scheduler's live preview didn't react to unsaved module toggles. Both fixed, verified live, merged to main.
- [x] **Lead Researcher History tab self-polling (2026-07-10, Session 60)** — Sam reported changing tabs "stops" an in-progress analysis. Root cause: local component state destroyed on tab switch; backend job unaffected. Fix: History tab now polls every 8s while anything is "running," stops once nothing is pending. Spec written first (`docs/specs/2026-07-10-lead-researcher-history-polling.md` in ai-employees-app), then implemented + verified live.
- [x] **Competitor Agent "Discovery Failed" root-caused (2026-07-09/10, Session 60)** — confirmed via the DB's actual `error_message`: a real `401 Incorrect API key` from OpenAI on the production server (Sam's real business, Divinity DJs) — a deployed-server credentials issue, not a code bug. Rahul has since updated the key on production; **not yet re-verified with a fresh retry.**
- [x] **Market Agent generic-reports issue root-caused (2026-07-09/10, Session 60)** — `businesses.type` (the only industry signal Market Agent had) is often just a vague placeholder like "other." Proposal + meeting brief written (`docs/market-agent-industry-relevance-proposal.md`, `docs/market-agent-meeting-brief.md`). Sam's response: use the Branding tab mockup instead — see Business Branding feature below.
- [x] **Business Branding feature — built end-to-end (2026-07-12/13, Session 60)** — new `business_branding` table (applied live), backend CRUD, Market Agent now uses `target_niche` for industry context (safe fallback for businesses without it), full `BrandingTab.tsx` UI (Color Palette/Typography, Core Brand, Communication Strategy, Competitive Analysis, Market Insights). Spec: `docs/superpowers/specs/2026-07-10-business-branding-section.md`.
- [x] **Full Sales Employee + Business Branding re-test — 26/26 pass (2026-07-13/14, Session 61)** — `docs/sales-employee-full-retest-2026-07-13.md`, 5 parallel Canary sessions. All modules pass. Headline: TC-MA-INTEGRATION-001 confirms with quoted evidence that Market Agent reports now genuinely reflect the business's real niche (real named competitors, trade-press citations), not generic content — backend logs prove the fetch-order wiring works. One regression found+fixed same session: Report Scheduler send-test failed due to an expired Gmail OAuth token (not a code bug) — reconnected, re-verified `sent:true`. 2 minor non-blocking findings logged (cosmetic DOM-nesting warning; Competitor Agent Jina extraction gap on a real site). **Merged `feature/business-branding` → main in both repos** (clean fast-forward, backend health + frontend build verified) — no longer pending.
- [x] **Market Agent report-view + custom-prompt edit flow implemented (2026-07-16, Session 63)** — added a `View Report` action on every card plus a full report dialog showing the full insight, generated timestamp, stored prompt, and all available source links. Custom cards can now reopen the prompt in edit mode from either the card footer or the report dialog. Backend added `PATCH /sales/market-agent/custom-analysts/{id}` plus `custom_analyst_id` and `prompt_used` on card responses; migration `20260716000000_market_agent_card_prompt_linkage.sql` persists the linkage and backfills uniquely matchable historical custom cards. Verified with frontend typecheck, backend syntax parse, and clean IDE lints.
- [ ] **App-wide toast bug found while verifying Business Branding (2026-07-13, Session 60)** — every `toast.success()`/`toast.error()` call via the `sonner` package is silently a no-op across the ENTIRE app. Confirmed NOT caused by session 60's work (an old, untouched Report Scheduler save button has the identical symptom). Ruled out: stale Docker image, stale Vite dep cache, duplicate `sonner` installs, console errors, `next-themes` provider issues. Root cause not yet found — needs real breakpoint debugging inside `node_modules`, out of scope for black-box browser testing. Underlying save operations all work correctly; only the confirmation pop-up is broken. **Needs a dedicated fix.**
- [ ] **Separate real bug found while investigating the above, fixed but NOT YET APPLIED to the DB:** `updated_at` was never actually bumped on any `.update()` call across all 4 new Sales Employee modules (~28 call sites) — every other table in this schema already has a `public.handle_updated_at()` trigger for this, the 8 new tables were just missing it. Fixed via migration `20260707000002_sales_employee_updated_at_triggers.sql` (DB-level trigger, not patching each Python call site — matches existing codebase convention). **Migration written and committed, needs `supabase db push`** — verified via a live test that it is NOT yet firing (checked before this note was written).
...
- [x] **Migration applied** (Rahul, 2026-07-02).
- [x] **Bug found + fixed: toggle was invisible for everyone.** Queried the DB directly — **all 6 businesses have `stripe_subscription_id = NULL`, including Sam's own "AI Employees inc."** Nobody has ever gone through real Stripe Checkout (makes sense, free during beta). The Add-ons card was nested inside `{hasActiveSub && ...}`, so it never rendered for anyone. Fixed (commit `4ff1a33`): card now always renders, disabled with "Subscribe to a plan above to enable add-ons" when there's no base plan — a real Stripe constraint (can't attach a line item to a subscription that doesn't exist), not hidden.
- **⚠️ Important consequence for later:** since literally no business has a real Stripe subscription, flipping `EXEC_AGENT_ADDON_ENFORCED` on (or building the bigger whole-product enforcement gap) would lock out every business immediately, including Sam's — until businesses actually go through Checkout. Don't flip enforcement on without a real subscription flow in place first.
- [ ] **Not yet live-tested** — needs a real test: with no base plan, confirm toggle is disabled with the explanatory note; with a base plan (create one via Stripe test mode), confirm toggle enables/disables and creates/removes the Stripe subscription item; confirm `/dashboard/executive` still accessible either way (enforcement is off by default).
- [ ] `STRIPE_EXEC_AGENT_PRICE_ID` still not configured — toggle will 503 "not configured" until Sam names a price and it's created in the Stripe dashboard. This is expected/correct per the ADR, not a bug.
...
- **Decided: yes, the toggle should actually gate access** (option 2 in the discussion) — not just track billing with no enforcement. Rahul confirmed this is how it should be built and tested.
- **⚠️ Bigger finding from this discussion — logged, NOT being fixed right now (deliberately deferred, don't rush):** searched the entire backend — **zero subscription/billing enforcement exists anywhere in the product**, for ANY feature, not just the Executive Agent. The only place `has_subscription`/`stripe_subscription_status`/`subscription_call_limit` are even referenced is the Billing page's own display. A business with a canceled subscription, a lapsed trial, or no subscription at all can currently still make calls, book appointments, and use every feature — the "80% of minutes used" / "limit reached" warnings on the Billing page are cosmetic text, not an actual block on anything. This predates the Executive Agent work; it's a whole-product gap.
- **Agreed architecture for whenever this IS built** (both the Exec Agent gate and, later, the bigger whole-product gate):
  1. **Frontend:** extend `AuthContext.tsx`'s existing login/refresh bootstrap (which already fetches `profile`/`roles`/`permittedPaths` once and lets every page read from it via `canAccess()`) to also include subscription/billing state. This avoids "flash of the page, then blocked" — the block decision is known before the user even navigates, consistent with how role-based access already works. Per-page/incremental billing checks were considered and rejected for this — billing state doesn't change often enough to need per-page freshness, and it'd create two inconsistent gating systems (bootstrap for roles, per-page for billing).
  2. **Backend — MANDATORY, not optional:** whatever the frontend does, `POST /executive/session` (and any future gated endpoint) must independently check subscription/add-on status server-side and reject if not active. Frontend-only gating can be bypassed by anyone calling the API directly — it's a UX nicety, not real enforcement. Both layers are required.
- **Next step when we actually pick this up:** scope the Executive-Agent-specific slice of this (small: one endpoint + one page) separately from the whole-product gap (large: every feature, every endpoint) — don't conflate the two pieces of work.
...
- [x] **New Stripe price IDs** — verified session 62 (2026-07-14): `STRIPE_GROWTH_PRICE_ID` + `STRIPE_PRO_PRICE_ID` both already set in `backend/.env` (alongside Starter + Enterprise). Stale checkbox — this was done in an earlier session without being marked.
