...
**Phase B — remaining slices:** slice 2 = `appointment_list` Cancel/Reschedule (destructive → in-card confirm); slice 3 = `email_detail` (read-only). Both reuse the `card_action` protocol built in WS11. **Test:** "list my emails" / "what's on my calendar today" → card renders, Remi summarises briefly.
