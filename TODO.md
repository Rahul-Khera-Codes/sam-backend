...
- [x] **Idle-session auto-disconnect.** ✅ DONE 2026-07-02 — listens to LiveKit's built-in `user_state_changed`, hangs up via `ctx.room.disconnect()` after 3 min of sustained "away" state. Commit `f8972ee`. **Live-verify pending:** confirm text-only (mic-off) sessions also trigger `user_state_changed` correctly — the away-timer is primarily audio/VAD-driven; text exchanges should still reset it via agent_state transitions but hasn't been confirmed live.
