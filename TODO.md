...
**WS7 — `list_emails` N+1 perf:** ✅ DONE in code (commit `3fd5d03`), pending live verify. Fetch token once + `asyncio.gather` the per-message gets + optional token param on helpers. Was ~11s → expect ~1–2s. Spec `docs/superpowers/specs/2026-06-24-executive-agent-list-emails-perf.md`. **Test:** "list my emails" should now return fast.
**Cards not rendering = expected:** WS3 cards are spec-only, not implemented. `list_emails` returns text today; building WS3 Phase A turns it into the `email_list` card.
