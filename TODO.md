...
**WS3 — rich cards Phase A:** ✅ info cards DONE in code (both repos), pending live verify. `email_list` (list_emails) + `calendar_schedule` (get_schedule) emit via `_send_card` → render as cards above the input (`AgentCardView` registry + text fallback); agent speaks a one-line summary. Envelope `{type:card,card,id,data}` + `card_dismiss`. **Deferred (Phase A.2):** migrate existing email_draft/calendar_event PREVIEWS into the card envelope (they still work via the preview panel). **Phase B:** email_detail, free_slots pick-to-book, appointment_list actions. **Test:** "list my emails" / "what's on my calendar today" → card renders, Remi summarises briefly.
