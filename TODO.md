...
- [x] Phase-2 avatar = HeyGen **LiveAvatar** SHIPPED (session 54). `livekit-agents[openai,liveavatar]~=1.4`. Production mode. `AvatarSession` + graceful degradation try/except (orb fallback). Double-audio fix. Female avatar `073b60a9…` + voice `marin`. 288px circle. Confirmed in logs + UI. **Requires Docker rebuild:** `docker compose up -d --build sam-executive-agent`. Known: ~2min HeyGen session timeout → orb fallback (graceful). Spec: `docs/superpowers/specs/2026-06-30-executive-agent-heygen-avatar-poc.md`.
