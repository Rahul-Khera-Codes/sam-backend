...
- [ ] **Real bug from Yuvraj's own testing (2026-07-07), root-caused + partially fixed:** 2 Lead Researcher lookups failed with "Timed out waiting for Apify — the webhook never arrived." Root cause confirmed via Apify's webhook-dispatch API: the actual Apify scrapes **succeeded**, but delivery got HTTP 404 **from ngrok itself** (tunnel not running at that moment) — not a code bug. Both lookups manually recovered from Apify's already-succeeded dataset (no data lost) — `6eb0698e-...` (Arun Kumar) and `efa66db2-...` (Yuvraj Singh) now show `status: completed`. **Still needed:** a persistent ngrok static domain (Rahul needs to reserve one at dashboard.ngrok.com/domains — waiting on this) so the tunnel URL survives restarts; real fix is production deploy (already tracked above).
