...
- [x] **#7 Edit + copy buttons on transcript bubbles.** ✅ DONE 2026-07-02 (frontend only, `ai-employees-app`). Copy button on every bubble, user + agent (commit `1321b0f`). Edit button on user bubbles only — since the model already processed the original spoken turn, "edit" resends the corrected text as a new message via the existing `sendMessage` path rather than rewriting history (commit `ccdf233`). Needs a live test.
