...
- [ ] **Follow-up gaps found post-verification (2026-07-06)** — before calling this production-ready: (1) `lead_lookups` was missing RLS that every comparable table has — migration written (`20260706000001_lead_lookups_rls.sql`), **not yet applied to the DB**; (2) no duplicate-request protection — an accidental double curl during testing started 2 paid Apify runs, need to skip starting a new run if one's already pending/running for the same business+URL; (3) no stuck-job handling if Apify's webhook never fires; (4) production still needs real (non-personal, non-ngrok) Apify + webhook env values. **Next:** fix 1-3, then hand the API contract to Yuvraj so he can start the Lead Researcher UI.
