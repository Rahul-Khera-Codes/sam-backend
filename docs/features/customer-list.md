...
## QA follow-ups (AIE-61, 2026-09-08)

Addressed the round of Linear comments left after the initial "Ready for QA" pass:

- **Executive Agent (Remi) can now access the customer list.** `sam-backend/agent/executive_agent.py` gained two new tools: `search_customers` (name/email/phone search, scoped to `business_id`, no location filter — Remi is business-wide) and `get_customer` (full record by id: contact info, do-not-contact flag, notes, last 5 appointments). `EXECUTIVE_INSTRUCTIONS` now tells Remi to resolve a customer's email via these tools before `draft_email`/`draft_reply` rather than guessing one, and to get explicit owner confirmation before contacting anyone flagged do-not-contact. Two new frontend card types (`customer_list`, `customer_detail`) render the results in `ai-employees-app/src/components/executive/AgentCardView.tsx` (types in `useExecutiveSession.ts`); the detail card has an "Email" button (hidden if do-not-contact or no email on file) that raises the same `card_action` → synthetic-prompt path as the existing `reply_email` button (handled in `executive_agent.py`'s `_on_data`, new `email_customer` action).
- **Bulk actions on the Customer List table.** `CustomerList.tsx` has a checkbox column (+ header select-all-on-page) and an action bar that appears when rows are selected, with "Mark do not contact" and "Delete" (delete goes through a confirm dialog, mirroring the existing single-row delete). New `bulkDeleteCustomers`/`bulkSetDoNotContact` hook methods in `useCustomers.ts` do one `.in("id", ids)` Supabase call each rather than looping single-row calls.
- **Import dialog widened** (`max-w-3xl`) so the CSV preview table's Telephone column isn't clipped — was `max-h-[85vh] overflow-y-auto` with no width class, so it fell back to the default `DialogContent` width.
- **New Appointment modal: separate First/Last Name fields.** The single "Client Name" input in `Calendar.tsx`'s create-appointment dialog is now two inputs (`newClientFirstName`/`newClientLastName`, local component state — not part of the shared `AppointmentForm` type). They're joined into one string at submit time and stored in `client_name` exactly as before; `resolve_or_create_customer` in `customers_service.py` already re-splits that string into `customers.first_name/last_name`, so no backend change was needed. Scoped to the *create* dialog only, per the ticket comment — the Edit Appointment dialog's `clientName` field (which is populated from existing `client_name` rows) was intentionally left untouched.

Not done from the original comment thread: the "search a client to open their record inside this section" comment (posted before the feature was built) is considered already satisfied by the shipped search bar + Edit dialog — no separate autocomplete was added to the Calendar "New Appointment" modal's name fields (still a possible future follow-up, as noted below).
