...
- [ ] **Outlook integration (2026-09-03) — 3 steps left before it works end-to-end:** (1) fix the Azure app's registered redirect URI — client's screenshot showed `https://portal.aiemployeesinc.com//integrations/outlook/callback` with a double slash, confirmed as a typo, must be single-slash to match `OUTLOOK_REDIRECT_URI`; (2) paste the real `MICROSOFT_CLIENT_SECRET` value into `backend/.env` — only a truncated preview was visible in the client's screenshot; (3) run `supabase db push` for `20260903000000_outlook_tokens.sql` (holding for explicit go-ahead first). See `docs/features/outlook-integration.md` for full spec — this is connect-flow only, Outlook is NOT wired up as an actual email sender yet (Gmail still handles all transactional email).
...
- `20260903000000_outlook_tokens.sql` (ai-employees-app) — `outlook_tokens` table for the new Outlook OAuth integration. **Not yet pushed** — needs explicit confirmation before `supabase db push` per user instruction (see `memory/feedback_ai_employees_workflow.md`), overriding this repo's default "push immediately" rule.
