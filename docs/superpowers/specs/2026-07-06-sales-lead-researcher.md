...
1. **Missing RLS on `lead_lookups`** — every comparable business-scoped table in this schema (e.g. `business_documents`) has Row Level Security enabled even though the backend only reads via the service-role key; this table didn't. **Fixed and applied:** `20260706000001_lead_lookups_rls.sql` pushed to the DB. Verified by comparing an anon-key read (0 rows) against a service-role read (3 real rows) — RLS is actively enforcing.
2. **No duplicate-request protection** — nothing stopped two rapid `POST /lookup` calls for the same business+URL from starting two paid Apify runs. Directly observed: a 3rd row appeared during testing from an accidental duplicate curl execution. **Fixed** (commit `72c3a0e`): `POST /lookup` now returns the existing `pending`/`running` lookup instead of starting a new run, for the same `business_id` + `linkedin_url`. Verified directly against the running container.
3. **No stuck-job handling** — if Apify's webhook never fires (network blip, actor timeout), a lookup would sit in `running` forever with no retry or timeout. **Fixed** (commit `72c3a0e`): a lookup still `running` past 10 minutes (real runs take ~2-3 min) is marked `failed` with a clear error message on next read (`GET /lookup/{id}` or `/history`). Verified with an artificially-aged row.
...
## Handoff
API contract for the frontend written: `docs/lead-researcher-api-contract.md`. Yuvraj can start the Lead Researcher UI against this.
